...
const PROCESS = [
  { n: '01', t: 'Storyboard', d: 'We script the scroll — scene by scene — before any code.', icon: '📝' },
  { n: '02', t: 'Design', d: 'Visual direction and a scrub-able prototype you approve.', icon: '🎨' },
  { n: '03', t: 'Build', d: 'GSAP + React dev in weekly sprints. You feel it working.', icon: '⚙️' },
  { n: '04', t: 'Measure', d: 'Ship, then read the scroll heatmaps and improve.', icon: '📈' },
]
const Process = () => (
...
const Testimonials = () => (
  <section className="quotes"><div className="wrap">
    ...
    <div className="quotes-grid">{QUOTES.map((q, i) => <Reveal key={i} delay={i * 0.08}><figure className="quote"><div className="stars">★★★★★</div><blockquote>{q.q}</blockquote><figcaption><b>{q.n}</b><span>{q.r}</span></figcaption></figure></Reveal>)}</div>
  </div></section>
)
...
const CTA = () => {
  ...
  return (
    <section className="cta" id="contact"><div className="wrap cta-inner">
      ...
      </motion.form> : <motion.div className="cta-done" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}><BadgeCheck size={44} /><h3>Brief received 🎉</h3><p>We'll reply within one business day with your scroll script + quote.</p></motion.div>}
